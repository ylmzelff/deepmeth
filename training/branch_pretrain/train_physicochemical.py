...
CHECKPOINT_SUBDIR = ACTIVE_CHECKPOINT_DIR.parent / "physicochemical_only"
LAST_CHECKPOINT_PATH = CHECKPOINT_SUBDIR / "last_checkpoint.pt"
BEST_CHECKPOINT_PATH = CHECKPOINT_SUBDIR / "best_model.pt"

HISTORY_PATH = ACTIVE_RESULTS_DIR / "training_history_physicochemical_pretrain.json"


class PhysicochemicalOnlyModel(nn.Module):
    """Physicochemical branch + a small linear head, for standalone pretraining only."""

    def __init__(self, head_hidden_dim: int, head_dropout_prob: float):
        super().__init__()
        self.physchem_branch = CNNNet_PhyChemDi(dropout_prob=PHYSCHEM_DROPOUT)
        self.head = nn.Sequential(
            nn.Linear(PHYSICOCHEMICAL_CNN_OUTPUT_DIM, head_hidden_dim),
            nn.ReLU(),
            nn.Dropout(head_dropout_prob),
            nn.Linear(head_hidden_dim, 1),
        )

    def forward(self, physchem_input: torch.Tensor) -> torch.Tensor:
        return self.head(self.physchem_branch(physchem_input)).squeeze(1)
...
def main() -> None:
    ...
    print("Pretraining physicochemical branch alone")
    ...
    print("\n[2/3] Building model")
    model = PhysicochemicalOnlyModel(
    ...
    print("\nPhysicochemical pretraining completed.")
